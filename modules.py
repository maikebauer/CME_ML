...
class ResidualConv(nn.Module):
    def __init__(self, input_dim, output_dim, stride, padding, drop_p=0.0):
...
class Squeeze_Excite_Block(nn.Module):
    def __init__(self, channel, reduction=16, drop_p=0.0):
...
class ASPP(nn.Module):
    def __init__(self, in_dims, out_dims, rate=[6, 12, 18], drop_p=0.0):
...
class AttentionBlock(nn.Module):
    def __init__(self, input_encoder, input_decoder, output_dim, drop_p=0.0):
        super(AttentionBlock, self).__init__()

        self.conv_encoder = nn.Sequential(
            # nn.BatchNorm3d(input_encoder),
            nn.GroupNorm(1, input_encoder),
            nn.ReLU(),
            nn.Dropout3d(drop_p),
